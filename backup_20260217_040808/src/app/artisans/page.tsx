...
export default function ArtisansPage() {
  // ✅ Initialize artisans data directly with useMemo
  const allArtisans = useMemo(() => getUniqueArtisans(), []);
  
  // ✅ Set initial state directly
  const [artisans] = useState<Artisan[]>(allArtisans);
  const [featuredArtisan, setFeaturedArtisan] = useState<Artisan | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedSpecialty, setSelectedSpecialty] = useState<string>("all");
  const [isMounted, setIsMounted] = useState(false);

  // ✅ Only set featured artisan (client-side only) and mount status
  useEffect(() => {
    if (allArtisans.length > 0 && !featuredArtisan) {
      const randomIndex = Math.floor(Math.random() * allArtisans.length);
      setFeaturedArtisan(allArtisans[randomIndex]);
    }
    setIsMounted(true);
  }, [allArtisans, featuredArtisan]); // Added featuredArtisan to dependencies
