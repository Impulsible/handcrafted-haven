...
// Live Market Stats Component with Persistence - FIXED HYDRATION
const MarketStats = () => {
  ...
  const [timeUntilUpdate, setTimeUntilUpdate] = useState(60 * 60);
  ...
  useEffect(() => {
    ...
    setTimeUntilUpdate(Math.max(0, (60 * 60) - elapsedSeconds));
  }, []);

  // Update formatted time only on client
  useEffect(() => {
    if (isMounted) {
      setFormattedTime(stats.lastUpdated.toLocaleTimeString());
    }
  }, [stats.lastUpdated, isMounted]);

  // Save stats to localStorage whenever they change
  useEffect(() => {
    if (isMounted) {
      localStorage.setItem('marketStats', JSON.stringify({
        ...stats,
  ...
  return (
    <div className="bg-gradient-to-r from-primary/5 via-secondary/5 to-accent/5 rounded-2xl p-4 sm:p-6 border border-primary/10">
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-medium text-white bg-emerald-600 px-2 py-1 rounded-full">
                  {item.change}
                </span>
              </div>
      ...
      <div className="mt-3 text-xs text-center text-muted-foreground">
        Last updated: {isMounted ? formattedTime : "..."} • Stats persist across page reloads
      </div>
    </div>
  );
};
...
const heroSlides = [
  ...
  return (
    <div className="relative h-[500px] sm:h-[550px] md:h-[600px] lg:h-[650px] w-full overflow-hidden rounded-2xl">
        ...
        >
          ...
          <div className="relative z-20 container mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center">
            <div className="max-w-2xl text-white">
              <div className="inline-flex items-center px-3 py-1 bg-amber-600/90 backdrop-blur-sm rounded-full mb-4 sm:mb-6">
                <Sparkles className="w-4 h-4 mr-2 text-white" />
                <span className="text-xs sm:text-sm font-medium text-white">Artisan Marketplace</span>
              </div>
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-heading font-bold mb-3 sm:mb-4 text-white">
                {slide.title}
              </h1>
              ...
              <Link href={slide.link}>
                <Button className="bg-amber-600 hover:bg-amber-700 text-white px-6 sm:px-8 py-5 sm:py-6 text-base sm:text-lg group border-0">
                  {slide.cta}
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-black/50 hover:bg-black/70 text-white transition-colors backdrop-blur-sm"
      ...
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-black/50 hover:bg-black/70 text-white transition-colors backdrop-blur-sm"
...
// CategoryCarousel, QuickFilters, Pagination components unchanged
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                  ...
                                  {product.soldCount && (
                                    <span className="text-xs text-white bg-emerald-600 px-2 py-0.5 rounded-full">
...
// Add these missing components at the bottom
