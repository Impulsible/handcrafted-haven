...
// Live Market Stats Component with Persistence - Fixed for Hydration
const MarketStats = () => {
  ...
  // Set mounted state and initialize timer only on client
  useEffect(() => {
    setMounted(true);
    ...
    const updateTimer = setInterval(() => {
      if (typeof window !== 'undefined') {
        ...
        setTimeUntilUpdate(formatTime(remainingSeconds));
      }
    }, 1000);
  ...
  // Don't render time until mounted on client
  const timeDisplay = mounted ? timeUntilUpdate : '...';
    ...
    <div className="bg-gradient-to-r from-primary/5 via-secondary/5 to-accent/5 rounded-2xl p-4 sm:p-6 border border-primary/10">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
          <span>Updates in {timeDisplay}</span>
        </div>
      </div>
